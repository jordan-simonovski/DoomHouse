/*
   ...
   6. LOOKUP TABLES (Pre-computed Floor Distances):
      ...
      To optimize this, we pre-calculate these values into `doomhouse_poll.dict_floor_dist`.
...
FROM (
    SELECT
        ...
        multiIf(
            ...
            CAST(bitOr(bitOr(bitShiftLeft(toUInt32(dictGet('doomhouse_poll.dict_tex_wall1_data', 'r', w_tex_idx) * base_shade), 0), bitShiftLeft(toUInt32(dictGet('doomhouse_poll.dict_tex_wall1_data', 'g', w_tex_idx) * base_shade), 8)), bitShiftLeft(toUInt32(dictGet('doomhouse_poll.dict_tex_wall1_data', 'b', w_tex_idx) * base_shade), 16)), 'UInt32'),
            toInt32(y) < draw_start,
            CAST(bitOr(bitOr(bitShiftLeft(toUInt32(dictGet('doomhouse_poll.dict_tex_ceiling_data', 'r', f_tex_idx) * floor_shade), 0), bitShiftLeft(toUInt32(dictGet('doomhouse_poll.dict_tex_ceiling_data', 'g', f_tex_idx) * floor_shade), 8)), bitShiftLeft(toUInt32(dictGet('doomhouse_poll.dict_tex_ceiling_data', 'b', f_tex_idx) * floor_shade), 16)), 'UInt32'),
            CAST(bitOr(bitOr(bitShiftLeft(toUInt32(dictGet('doomhouse_poll.dict_tex_floor_data', 'r', f_tex_idx) * floor_shade), 0), bitShiftLeft(toUInt32(dictGet('doomhouse_poll.dict_tex_floor_data', 'g', f_tex_idx) * floor_shade), 8)), bitShiftLeft(toUInt32(dictGet('doomhouse_poll.dict_tex_floor_data', 'b', f_tex_idx) * floor_shade), 16)), 'UInt32')
        ) AS final_color
    FROM (
        SELECT 
                    ...
                    FROM (
                        SELECT 
                            *, arrayMap(i -> (i - valid_x) / r_dir_x, steps) as d_x,
                            arrayMin(arrayMap((d, i) -> if(d > 0 AND d < 30 AND dictGet('doomhouse_poll.dict_map_data', 'val', toUInt32(floor(valid_y + r_dir_y * d) * MAP_W + floor(valid_x + r_dir_x * d + if(r_dir_x > 0, 0.005, -0.005)) + 1)) > 0, d, 999.0), d_x, steps)) as dist_x,
                            arrayMap(i -> (i - valid_y) / r_dir_y, steps) as d_y,
                            arrayMin(arrayMap((d, i) -> if(d > 0 AND d < 30 AND dictGet('doomhouse_poll.dict_map_data', 'val', toUInt32(floor(valid_y + r_dir_y * d + if(r_dir_y > 0, 0.005, -0.005)) * MAP_W + floor(valid_x + r_dir_x * d) + 1)) > 0, d, 999.0), d_y, steps)) as dist_y
                        FROM (
                            SELECT 
                            ...
                            FROM (
                                SELECT 
                                    ...
                                    if(dictGet('doomhouse_poll.dict_map_data', 'val', toUInt32(floor(try_y + if(try_y > old_y, 0.2, -0.2)) * MAP_W + floor(valid_x_inter) + 1)) = 0, try_y, old_y) as valid_y,
                                    valid_x_inter as valid_x
                                FROM (
                                    SELECT *, if(dictGet('doomhouse_poll.dict_map_data', 'val', toUInt32(floor(old_y) * MAP_W + floor(try_x + if(try_x > old_x, 0.2, -0.2)) + 1)) = 0, try_x, old_x) as valid_x_inter
        ...
        CROSS JOIN (
            SELECT number as y, toInt32(H - 1 - number) as dist_lookup_idx, dictGet('doomhouse_poll.dict_floor_dist', 'dist', toUInt32(dist_lookup_idx + 1)) as floor_dist
...
FROM (
    ...
    FROM (
        ...
        CROSS JOIN (
            SELECT number + H_QUARTER - 1 as y, toInt32(H - 1 - (number + H_QUARTER - 1)) as dist_lookup_idx, dictGet('doomhouse_poll.dict_floor_dist', 'dist', toUInt32(dist_lookup_idx + 1)) as floor_dist
...
FROM (
    ...
    FROM (
        ...
        CROSS JOIN (
            SELECT number + H_HALF - 1 as y, toInt32(number + H_HALF - 1) as dist_lookup_idx, dictGet('doomhouse_poll.dict_floor_dist', 'dist', toUInt32(dist_lookup_idx + 1)) as floor_dist
            FROM numbers(H_QUARTER + 2)
        ) AS v_lines
    ) AS sub
)
...
FROM (
    ...
    FROM (
        ...
        CROSS JOIN (
            SELECT number + H_HALF + H_QUARTER - 1 as y, toInt32(number + H_HALF + H_QUARTER - 1) as dist_lookup_idx, dictGet('doomhouse_poll.dict_floor_dist', 'dist', toUInt32(dist_lookup_idx + 1)) as floor_dist
            FROM numbers(H_QUARTER + 1)
        ) AS v_lines
    ) AS sub
)
