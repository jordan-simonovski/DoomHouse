...
class DOOMHouse:
    def __init__(self):
        self.window_name = "DOOMHouse (NON-STREAMING) - ClickHouse SQL Game Engine"
        ...
        try:
            ...
            self.client.command("CREATE DATABASE IF NOT EXISTS doomhouse_ns")
    ...
    def _setup_texture_resource(self, table_name, dict_name, texture_file):
        ...
        try:
            self.client.command(f"DROP TABLE IF EXISTS doomhouse_ns.{table_name}")
            self.client.command(f"""
                CREATE TABLE doomhouse_ns.{table_name} (
                    id UInt32,
                    r UInt8,
                    g UInt8,
                    b UInt8
            ...
            self.client.command(f"DROP DICTIONARY IF EXISTS doomhouse_ns.{dict_name}")
            self.client.command(f"""
                CREATE DICTIONARY doomhouse_ns.{dict_name} (
                    id UInt32,
                    r UInt8,
                    g UInt8,
                    b UInt8
                ...
                PRIMARY KEY id
                SOURCE(CLICKHOUSE(TABLE '{table_name}' DB 'doomhouse_ns'))
        ...
        print(f"💾 Initializing doomhouse_ns.{table_name} table with {len(tex_data)} pixels...")
        try:
            self.client.command(f"TRUNCATE TABLE doomhouse_ns.{table_name}")
            data = [
                [
                    i + 1,
            ...
            self.client.insert(f'doomhouse_ns.{table_name}', data)
            
            print(f"🔄 Reloading dictionary doomhouse_ns.{dict_name}...")
            self.client.command(f"SYSTEM RELOAD DICTIONARY doomhouse_ns.{dict_name}")
        except Exception as e:
    ...
    def cleanup_database(self):
        ...
        try:
            ...
            self.client.command("DROP VIEW IF EXISTS doomhouse_ns.render_materialized")
            self.client.command("DROP VIEW IF EXISTS doomhouse_ns.post_process_materialized")
            self.client.command("DROP VIEW IF EXISTS doomhouse_ns.render_materialized_top")
            self.client.command("DROP VIEW IF EXISTS doomhouse_ns.render_materialized_bottom")
            self.client.command("DROP VIEW IF EXISTS doomhouse_ns.post_process_materialized_top")
            self.client.command("DROP VIEW IF EXISTS doomhouse_ns.post_process_materialized_bottom")
            for i in range(1, 5):
                self.client.command(f"DROP VIEW IF EXISTS doomhouse_ns.render_materialized_{i}")
                self.client.command(f"DROP VIEW IF EXISTS doomhouse_ns.post_process_materialized_{i}")
            ...
            for d in dicts:
                self.client.command(f"DROP DICTIONARY IF EXISTS doomhouse_ns.{d}")
            ...
            for t in tables:
                self.client.command(f"DROP TABLE IF EXISTS doomhouse_ns.{t}")
            for i in range(1, 5):
                self.client.command(f"DROP TABLE IF EXISTS doomhouse_ns.rendered_frame_{i}")
                self.client.command(f"DROP TABLE IF EXISTS doomhouse_ns.rendered_frame_post_processed_{i}")
        except Exception as e:
    ...
    def initialize_game_data(self):
        ...
        self.execute_sql_script("non-streaming/SQL/create_source_tables.sql")
        self.execute_sql_script("non-streaming/SQL/create_dictionaries.sql")

    def initialize_tables(self):
        # Re-create tables to ensure schema matches
        sql_files = [
            "non-streaming/SQL/player_input_table.sql",
            "non-streaming/SQL/rendered_frame_table.sql",
            "non-streaming/SQL/rendered_frame_post_processed_table.sql",
            "non-streaming/SQL/render_view.sql",
            "non-streaming/SQL/post_process_view.sql",
        ]
    ...
    def push_input(self, target_x, target_y):
        try:
            ...
            self.frame_id += 1
            self.client.command(f"""
                INSERT INTO doomhouse_ns.player_input
        ...
        try:
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=4) as executor:
                future1 = executor.submit(
                    self.client.query,
                    "SELECT pos_x, pos_y, image_data FROM doomhouse_ns.rendered_frame_post_processed_1"
                )
                future2 = executor.submit(
                    self.client2.query,
                    "SELECT pos_x, pos_y, image_data FROM doomhouse_ns.rendered_frame_post_processed_2"
                )
                future3 = executor.submit(
                    self.client3.query,
                    "SELECT pos_x, pos_y, image_data FROM doomhouse_ns.rendered_frame_post_processed_3"
                )
                future4 = executor.submit(
                    self.client4.query,
                    "SELECT pos_x, pos_y, image_data FROM doomhouse_ns.rendered_frame_post_processed_4"
                )
